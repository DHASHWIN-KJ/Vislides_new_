...
class SocketService {
    ...
    onWhiteboardPDF(callback: (data: { pdfUrl: string }) => void) {
        if (this.socket) this.socket.on('whiteboard_pdf', callback);
    }
    ...
    emitWhiteboardPDF(sessionCode: string, pdfUrl: string) {
        if (this.socket) this.socket.emit('whiteboard_pdf', { sessionCode, pdfUrl });
    }
    ...
    offWhiteboardEvents() {
        if (this.socket) {
            ...
            this.socket.off('whiteboard_pdf');
        }
    }

    offWhiteboardContentEvents() {
        if (this.socket) {
            this.socket.off('whiteboard_draw');
            this.socket.off('whiteboard_clear');
            this.socket.off('whiteboard_pdf');
