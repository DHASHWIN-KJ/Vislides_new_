...
export const joinSession = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const session = await Session.findOne({ code: String(code).toUpperCase(), status: 'active' });
...
export const getSessionDetails = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const session = await Session.findOne({ code: String(code).toUpperCase() })
...
// @desc    Get all sessions a teacher has created
...
export const getTeacherSessions = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const sessions = await Session.find({
            teacher: userId,
            isQuerySession: { $ne: true } // Exclude the persistent query session
        })
            .sort({ createdAt: -1 });
